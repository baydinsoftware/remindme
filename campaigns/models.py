...
class Campaign(models.Model):
	...
	welcome_content = HTMLField(help_text="You can use the following shortcodes: {{name}} {{view-emails}} {{unsubscribe}} **{{deadline}} {{first-email}} {{months-away}}*deadline-only. You can directly paste HTML entities such as: &squ;")
...
class DeadlineCampaign(Campaign):
	...
	before_welcome_subject = models.CharField(max_length=100,help_text='This email is sent in place of standard welcome if user subscribes some amount of time before first email would be sent as determine by ontime margin above')
        before_welcome_content = HTMLField(help_text="You can use the following shortcodes: {{name}} {{view-emails}} {{deadline}} {{first-email}} {{unsubscribe}} {{months-away}} You can directly paste HTML entities such as: &squ;")
...
class DeadlineEmail(Email):
	...
	content_beginning = HTMLField(help_text='The content of email preceding any "add-ons" from other subscriptions. You can use the following shortcodes: {{name}} {{deadline}} {{view-emails}} {{unsubscribe}} You can directly paste HTML entities such as: &squ;')
	content_end = HTMLField(help_text='The content of email following the "add-ons" from other subscriptions. You can use the following shortcodes: {{name}} {{deadline}} {{view-emails}} {{unsubscribe}} You can directly paste HTML entities such as: &squ; ')
...
class DeadlineAddOn(models.Model):
	content = HTMLField(help_text='You can use the following shortcodes: {{name}} {{deadline}} {{view-emails}} {{unsubscribe}} You can directly paste HTML entities such as: &squ;')
...
class RelativeStartEmail(Email):
	...
	content = HTMLField(help_text="You can use the following shortcodes: {{name}} {{view-emails}} {{unsubscribe}} You can directly paste HTML entities such as: &squ;")
...
class FixedEmail(Email):
	...
	content = HTMLField(help_text="You can use the following shortcodes: {{name}} {{unsubscribe}} {{view-emails}}. You can directly paste HTML entities such as: &squ;")
