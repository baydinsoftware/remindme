...
def campaign(request,campaign_slug):
	...
	if type == RELATIVE:
		...
		if form.is_valid():
		    ...
		    unsubscribe_link = "<a href='%s'>unsubscribe</a>" % request.build_absolute_uri(reverse('campaigns:unsubscribe', args=(subscriber.id,)))
		    body = body.replace("{{unsubscribe}}",unsubscribe_link)
		    view_emails_here = "<a href='%s'>here</a>" % request.build_absolute_uri(reverse('campaigns:emails', args=(campaign.slug,)))
...
def emails(request,campaign_slug):
	...
	if type == FIXED:
		...
		for email in emails:
			...
			body = body.replace("{{unsubscribe}}",unsubscribe_link)
			view_emails_here = "<a href=''>here</a>"
			body = body.replace("{{view-emails}}",view_emails_here)
...
def unsubscribe(request,subscriber_id):
	...
	if request.method == 'GET':
		...
		view_emails_here = "<a href='%s'>here</a>" % request.build_absolute_uri(reverse('campaigns:emails', args=(slug,)))
		body = body.replace("{{view-emails}}",view_emails_here)
